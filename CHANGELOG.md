...
## [1.1.2] - 2026-01-17 - Web UI Access Fix 🔧

### Fixed
- **CLI entrypoint** now correctly uses Typer CLI instead of bypassing it (fixes #81)
- **Frontend assets** are now included in pip releases (`.gitignore` fix)
- **`mkv-match gui` command** now works correctly (alias for `serve`)

### Added
- **`mkv-match serve` command** to launch the Web UI server
- **Improved CLI help** with quick start guide in `--help` output
- **New documentation screenshots** for Web UI onboarding

### Changed
- Updated quickstart documentation with correct commands and screenshots

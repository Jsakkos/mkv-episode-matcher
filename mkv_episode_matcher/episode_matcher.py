...
from mkv_episode_matcher.utils import get_subtitles,cleanup_ocr_files
...
def process_show(season=None, dry_run=False,get_subs=False):
    ...
    if season is not None:
        mkv_files = [os.path.join(show_dir, season) for f in os.listdir(show_dir) if f.endswith(".mkv")]
...
def extract_srt_text(filepath):
    ...
    with open(filepath, 'r') as f:
        filepath = f.read()
    text_lines = [filepath.split('\n\n')[i].split('\n')[2:] for i in range(len(filepath.split('\n\n')))]
    # remove empty lines
    text_lines = [[line for line in lines if line] for lines in text_lines]
    # remove <i> or </i> tags
    text_lines = [[re.sub(r'<i>|</i>|', '', line) for line in lines] for lines in text_lines]
    # remove empty lists
...
def compare_and_rename_files(srt_files,reference_files,dry_run=False):
    ...
    for srt_text in srt_files.keys():
        ...
        for reference in reference_files.keys():
            ...
            mkv_file =os.path.join(parent_dir,os.path.basename(srt_text).replace('.srt', '.mkv' ))
            matching_lines = compare_text(reference_files[reference], srt_files[srt_text])
            if matching_lines >= int(len(reference_files[reference])*.1):
